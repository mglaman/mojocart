 ...
 */
function mojocart_install_tasks(&$install_state) {
  ...
  $panopoly_server = array(
    ...
    'default content callback' => 'mojocart_default_content',
  );
 ...
 */
function mojocart_form_install_configure_form_alter(&$form, $form_state) {
  // Hide some messages from various modules that are just too chatty.
  drupal_get_messages('status');
  drupal_get_messages('warning');

  // Set reasonable defaults for site configuration form
  $form['site_information']['site_name']['#default_value'] = 'MojoCart';
  $form['admin_account']['account']['name']['#default_value'] = 'admin';
  $form['server_settings']['site_default_country']['#default_value'] = 'US';
  $form['server_settings']['date_default_timezone']['#default_value'] = 'America/Chicago'; //Midwest <3
 ...
 * Implements hook_form_FORM_ID_alter()
 */
function mojocart_form_apps_profile_apps_select_form_alter(&$form, $form_state) {
 ...
 */
function mojocart_default_content(&$modules) {
