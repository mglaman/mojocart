...
system_install() {
  ...
  header Building MojoCart from current branch
  cd drupal
  ...
  cd profiles/mojocart/tests/behat
  composer install --prefer-source --no-interaction
  cd ../../../../../

  # Verify that all the .make files will work on Drupal.org.
  header Verifying .make file
  drush verify-makefile drupal/profiles/mojocart/drupal-org.make
  find drupal/profiles/mojocart/modules -name \*.make -print0 | xargs -0 -n1 drush verify-makefile
...
before_tests() {
  ...
  cd drupal
  drush si mojocart --db-url=mysql://root:@127.0.0.1/drupal --account-name=admin --account-pass=admin --site-mail=admin@example.com --site-name="MojoCart" --yes
...
run_tests() {
  ...
  cd drupal/profiles/mojocart/tests/behat
